...
class YamlEditor {
  ...
  void _initialize() {
    ...
    if (_contents != null) collectAliases(_contents!);
  }
  ...
  void update(Iterable<Object> path, Object value) {
    ...
    final keyOrIndex = pathAsList.last as dynamic;
  ...
  YamlNode? remove(Iterable<Object> path) {
    ...
    if (path.isEmpty) {
      edit = SourceEdit(0, _yaml?.length ?? 0, '');
  ...
  YamlNode? _traverse(Iterable<Object> path,
    ...
    for (var i = 0; i < pathList.length; i++) {
      final keyOrIndex = pathList[i] as dynamic;
  ...
  void _assertNoChildAlias(Iterable<Object> path, [YamlNode? node]) {
    ...
    if (node is YamlMap) {
      ...
      for (var i = 0; i < node.length; i++) {
        ...
        _assertNoChildAlias(updatedPath.cast(), node.nodes[keyList[i]]);
      }
    }
  }
  ...
  YamlNode? _deepModify(YamlNode? tree, Iterable<Object> path,
    ...
    final keyOrIndex = path.elementAt(subPath.length) as dynamic;
