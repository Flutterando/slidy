...
class PackageInstalationRepositoryImpl implements PackageInstalationRepository {
  ...
  PackageInstalationRepositoryImpl(
      {required this.pubspec, required this.client});
  ...
  Future<Either<SlidyError, SlidyProccess>> install(PackageName package) async {
    try {
      if (package.name.contains('@')) {
        ...
        package =
            package.copyWith(version: '^${await client.fetch(package.name)}');
      }
      pubspec.update(
      ...
      return Right(SlidyProccess(
          result: result ? 'Added ${package.name}: ${package.version}' : ''));
  ...
  Future<Either<SlidyError, SlidyProccess>> uninstall(
    ...
    try {
      ...
      final result = await pubspec.save();
      return Right(
          SlidyProccess(result: result ? 'Removed ${package.name}' : ''));
    } on SlidyError catch (e) {
      return Left(e);
    }
  }
